...
export function AccessRequestsTable({ requests }: { requests: AccessRequestRow[] }) {
  ...
  async function handleApprove(row: AccessRequestRow) {
    ...
    try {
      ...
      setBanner({ type: "success", text: "User created and welcome email sent." });
    } catch (err) {
      console.error("Welcome email failed:", err);
      setBanner({
        type: "error",
        text: `User created but welcome email failed. Temporary password: ${res.password}`,
      });
    }
  ...
  if (requests.length === 0) {
    return (
      <p className="mt-12 text-center text-sm text-[#8a9099]">No pending access requests.</p>
    );
  }
    ...
    <div className="mt-6">
      ...
      <div className="overflow-x-auto border border-[rgba(255,255,255,0.08)]">
        <table className="w-full min-w-[800px] border-collapse text-left text-sm">
          ...
          <tbody>
            {requests.map((row) => {
              const isDenied = (row.status ?? "pending").toLowerCase() === "denied";
              return (
                <Fragment key={row.id}>
                  <tr
                    className={`border-b border-[rgba(255,255,255,0.06)] ${isDenied ? "opacity-40" : ""}`}
                  >
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap justify-end gap-2">
                        <button
                          type="button"
                          disabled={pendingId === row.id || row.status === "approved" || isDenied}
                  ...
                  {expandedId === row.id && (
                    <tr className={isDenied ? "opacity-40" : ""}>
