      ...
      <div className="mx-auto max-w-[1400px] px-4 py-16 sm:px-6 sm:py-20 lg:px-12 lg:py-24">
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
          <div className={reversed ? "lg:order-2" : undefined}>
            ...
            {subtitle && (
              <p className="mt-5 text-base leading-relaxed text-[#8a9099] sm:text-lg">{subtitle}</p>
            )}
          </div>
          <ul className={`space-y-10 sm:space-y-11 ${reversed ? "lg:order-1" : ""}`}>
            {items.map((item, idx) => (
              ...
              >
                <p className="wh-label text-[#8a9099]">{String(idx + 1).padStart(2, "0")}</p>
                <h3 className="mt-2 font-display text-xl font-semibold tracking-tight text-white transition group-hover:text-white">
                  {item.title}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-[#8a9099] sm:text-base">{item.description}</p>
              </li>
            ))}
          </ul>
        </div>
