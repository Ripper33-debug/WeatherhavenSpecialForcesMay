...
export function Footer() {
  return (
    <footer data-global-footer className="border-t border-white/[0.08] bg-[#080a0c]">
      <div className="mx-auto max-w-[1400px] px-4 py-14 sm:px-6 lg:px-12">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4">
          <div className="lg:col-span-2">
            ...
            <p className="mt-1 font-mono text-[10px] font-medium tracking-[0.2em] text-[#8a9099]">{company.name}</p>
            <p className="mt-6 max-w-md text-sm leading-relaxed text-[#8a9099] sm:text-base">
              {company.tagline} Engineering-led delivery for U.S. and allied customers.
            </p>
          </div>
          {footerColumns.map((col) => (
            <div key={col.title}>
              <p className="wh-label">{col.title}</p>
              <ul className="mt-4 space-y-3">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-[#8a9099] transition hover:text-white"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-12 flex flex-col gap-4 border-t border-white/[0.08] pt-10 text-xs leading-relaxed text-[#8a9099] sm:flex-row sm:items-start sm:justify-between">
        ...
        <p className="mt-6 max-w-4xl border-l border-white/10 pl-5 text-[11px] leading-relaxed text-[#8a9099]">
